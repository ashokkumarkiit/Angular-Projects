...
export class ListOfStationsComponent implements OnInit {
  ...
  getMultiLineChart(stationName) {

    this.placesService.stationNameSelected = stationName;
    console.log(stationName);
    this.timeLimit = "1 HOUR";

    const realTimeSMALineChart = new RealTimeSMALineComponent(this.placesService);

    this.router.navigate(['/sma_chart']);

    realTimeSMALineChart.create_d3_chart(stationName,this.placesService,this.timeLimit);
